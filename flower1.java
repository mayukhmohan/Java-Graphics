...
public class flower1 extends Applet implements ActionListener {
    int OriginX, OriginY;
    int a,b,c,f_,g_,h_;
    ...
    public void paint(Graphics g) {
        ...
        try {
            a.midpointEllipse(g, sx, sy, rx, ry, 0);
            a.midpointEllipse(g,sx,sy,rx,ry,25);
            a.midpointEllipse(g,sx,sy,rx,ry,75);
        }catch(Exception e){}
    }
    @Override
    ...
    public class Algorithm {
        ...
        public void midpointEllipse(Graphics g, int sx, int sy, int a, int b,double theta){
            System.out.println(sx+" "+sy+" "+a+" "+b);
            int x=0, y=b;
            double a_sqr = Math.pow(a, 2);
            double b_sqr = Math.pow(b, 2);

            x = (int) (x*Math.cos(Math.toRadians(theta))+y*Math.sin(Math.toRadians(theta)));
            y = (int) (y*Math.cos(Math.toRadians(theta))-x*Math.sin(Math.toRadians(theta)));
